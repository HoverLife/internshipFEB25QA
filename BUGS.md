...
| BUG-6 | Некорректный sellerID в виде строки (например, «asas») возвращает HTTP код 404 | 1. Отправить запрос с sellerID, равным «asas». 2. Проверить HTTP код ответа. | Код 400 с сообщением о неверном формате sellerID. | HTTP код 404, без корректного сообщения об ошибке | 